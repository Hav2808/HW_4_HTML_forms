...
interestCheck.forEach(el => {
    el.addEventListener('change', function(event) {
        // поиск родителя у элемента, на котором произошло событие
        main = event.target.closest('.interest').querySelector('.interests_active'); 
        
        // Отмечаем все флажки внутри main, если выбран главный в группе элемент
        if (main !== null) {
        ...
        if(el.closest('.interests').classList.contains('interests_active')) {
            ...
            //console.log('allInterests:'+ allInterests.length);
            checkedInterests = el.closest('.interests').querySelectorAll('.interest__check:checked');
            //console.log('checkedInterests:'+ checkedInterests.length);
        } else {
